...
public class CommonController {
	...
	public Result<?> upload(@RequestParam("file")MultipartFile mf, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String dbpath = CommonUtil.upload(mf,bizPath);
	...
	public void view(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String localPath = CommonUtil.UPLOAD_PATH;
	...
	/**
	 * word预览，先转pdf,再输出pdf流
	 * @param request
	 * @param response
	 */
	@GetMapping("/wordPreview/**")
	public void wordPreview(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// ISO-8859-1 ==> UTF-8 进行编码转换
		String filePath = extractPathFromPattern(request);
		...
		try {
			...
			String downloadFilePath = CommonUtil.UPLOAD_PATH + File.separator + filePath;
			File file = new File(downloadFilePath);
			if (file.exists()) {
				inputStream = new BufferedInputStream(new FileInputStream(file));
				outputStream = response.getOutputStream();
				WordToPDF.wordConverterToPdf(inputStream, outputStream, new HashMap<>());
				response.flushBuffer();
			} else {
			...
			throw new Exception("文件预览失败" + e.getMessage());
			// e.printStackTrace();
		} finally {
			if (inputStream != null) {
				try {
