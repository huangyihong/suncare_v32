...
public class JwtUtil {
	...
	// Token有效期30分钟（用户登录过期时间是此时间的两倍，以token在reids缓存时间为准）
	//过有效期 并且登录没过期，刷新有效期，刷新登录过期时间
	// 改为1.5小时
	public static final long EXPIRE_TIME = 90 * 60 ;
	 ...
	 * @return token中包含的用户名
	 */
	public static String getPropValue(String token,String propName) {
		try {
			DecodedJWT jwt = JWT.decode(token);
			return jwt.getClaim(propName).asString();
		} catch (JWTDecodeException e) {
			return null;
		}
	}
	 ...
	 * @return 加密的token
	 */
	public static String sign(String username, String secret, String key) {
	...
	public static String sign(String username, String secret, String key, String dataSource,String systemCode) {
		Date date = new Date(System.currentTimeMillis() + EXPIRE_TIME * 1000);
		Algorithm algorithm = Algorithm.HMAC256(secret);
		// 附带username信息  数据来源
		return JWT.create()
				.withClaim("username", username)
				.withClaim("dataSource",dataSource)
				.withClaim("systemCode",systemCode)
				.withClaim("key",key)
				.withExpiresAt(date)
				.sign(algorithm);
