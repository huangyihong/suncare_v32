...
@Service("diseaseDiagAuditService")
@Transactional
public class MedicalDiseaseDiagAuditServiceImpl implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalDiseaseDiag> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalDiseaseDiag> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalDiseaseDiag recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalDiseaseDiag recordBean,List<MedicalDiseaseDiag> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalDiseaseDiag审核数据修改
	private MedicalDiseaseDiag auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalDiseaseDiag recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalDiseaseDiag.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalDiseaseDiag recordBean = (MedicalDiseaseDiag)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalDiseaseDiag  recordBean,List<MedicalDiseaseDiag> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalDiseaseDiag(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalDiseaseDiag(MedicalDiseaseDiag recordBean, MedicalAuditLog oldBean, List<MedicalDiseaseDiag> updateList, List<String> deleteList) {
