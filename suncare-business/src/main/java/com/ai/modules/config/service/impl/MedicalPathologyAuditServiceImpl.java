 ...
 * @Version: V1.0
 */
@Service("pathologyAuditService")
@Transactional
public class MedicalPathologyAuditServiceImpl  implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalPathology> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalPathology> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalPathology recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalPathology recordBean,List<MedicalPathology> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalPathology审核数据修改
	private MedicalPathology auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalPathology recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalPathology.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalPathology recordBean = (MedicalPathology)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalPathology  recordBean,List<MedicalPathology> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalPathology(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalPathology(MedicalPathology recordBean, MedicalAuditLog oldBean, List<MedicalPathology> updateList, List<String> deleteList) {
