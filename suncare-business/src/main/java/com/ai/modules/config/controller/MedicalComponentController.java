...
import com.ai.modules.config.service.IMedicalComponentService;
...
@Api(tags="成分表")
@RestController
@RequestMapping("/config/medicalComponent")
public class MedicalComponentController extends JeecgController<MedicalComponent, IMedicalComponentService> {
	@Autowired
	private IMedicalComponentService service;
	 ...
	 * 分页列表查询
	 *
	 * @param medicalComponent
	 * @param pageNo
	 * @param pageSize
	 * @param req
	 * @return
	 */
	@AutoLog(value = "成分表-分页列表查询")
	@ApiOperation(value="成分表-分页列表查询", notes="成分表-分页列表查询")
	@GetMapping(value = "/list")
	public Result<?> queryPageList(MedicalComponent medicalComponent,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) {
		QueryWrapper<MedicalComponent> queryWrapper = QueryGenerator.initQueryWrapper(medicalComponent, req.getParameterMap());
		Page<MedicalComponent> page = new Page<MedicalComponent>(pageNo, pageSize);
		IPage<MedicalComponent> pageList = service.page(page, queryWrapper);
	 ...
	 * @throws Exception
	 */
	@AutoLog(value = "成分表-管理分页列表查询")
	@ApiOperation(value="成分表-管理分页列表查询", notes="成分表-管理分页列表查询")
	@GetMapping(value = "/manageList")
	public Result<?> queryPageManageList(MedicalComponent medicalComponent,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) throws Exception {
		QueryWrapper<MedicalComponent> queryWrapper = service.getQueryWrapper(medicalComponent, req);
		Page<MedicalComponent> page = new Page<MedicalComponent>(pageNo, pageSize);
		IPage<MedicalComponent> pageList = service.page(page, queryWrapper);
		return Result.ok(pageList);
	}
	 ...
	 * @return
	 */
	@AutoLog(value = "成分表-添加")
	@ApiOperation(value="成分表-添加", notes="成分表-添加")
	@PostMapping(value = "/add")
	@RequiresPermissions("basicDataManage:add")
	public Result<?> add(@RequestBody MedicalComponent bean) {
		...
		if(StringUtils.isNotBlank(id)){//修改(新增待审核状态)
			service.onlyUpdateMedicalComponent(bean);
			return Result.ok("修改成功！");
		}else{//第一次新增
			String medicalComponentId = IdUtils.uuid();
        	bean.setId(medicalComponentId);
			...
			service.saveMedicalComponent(bean);
		}
	 ...
	 * @return
	 */
	@AutoLog(value = "成分表-修改")
	@ApiOperation(value="成分表-修改", notes="成分表-修改")
	@PutMapping(value = "/edit")
	@RequiresPermissions("basicDataManage:edit")
	public Result<?> edit(@RequestBody MedicalComponent bean) {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		bean.setActionStaff(user.getId());
		bean.setActionStaffName(user.getRealname());
      	bean.setActionTime(new Date());
      	bean.setUpdateTime(new Date());
		service.updateMedicalComponent(bean);
		return Result.ok("修改成功!");
	}
	 ...
	 * @return
	 */
	@AutoLog(value = "成分表-通过id查询")
	@ApiOperation(value="成分表-通过id查询", notes="成分表-通过id查询")
	@GetMapping(value = "/queryById")
	public Result<?> queryById(@RequestParam(name="id",required=true) String id) {
		MedicalComponent medicalComponent = service.getById(id);
		return Result.ok(medicalComponent);
	}
	 ...
	 * @return
	 */
   	@AutoLog(value = "成分表-判断code是否重复 ")
	@ApiOperation(value="成分表-判断code是否重复 ", notes="成分表-判断code是否重复 ")
	...
	@PutMapping(value = "/delMedicalComponent")
	@RequiresPermissions("basicDataManage:del")
	public Result<?> delMedicalComponent(@RequestBody MedicalComponent bean) {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		bean.setActionStaff(user.getId());
		bean.setActionStaffName(user.getRealname());
      	bean.setActionTime(new Date());
      	bean.setDeleteTime(new Date());
		service.delMedicalComponent(bean);
		return Result.ok("删除操作成功");
	}

	@AutoLog(value = "全部删除操作")
	@ApiOperation(value="全部删除操作", notes="全部删除操作")
	@GetMapping(value = "/delAllMedicalComponent")
	@RequiresPermissions("basicDataManage:delAll")
   	public Result<?> delAllMedicalComponent(HttpServletRequest req,MedicalComponent medicalComponent) throws Exception {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		try {
			QueryWrapper<MedicalComponent> queryWrapper = service.getQueryWrapper(medicalComponent, req);
			MedicalComponent bean = new MedicalComponent();
			...
			bean.setActionStaffName(user.getRealname());
			bean.setActionTime(new Date());
			//service.delAllMedicalComponent(queryWrapper,bean);
			return importTaskService.saveBatchTask("MEDICAL_COMPONENT","成分表全部删除",bean, queryWrapper,
					(b,q)->{
						try {
							return this.service.delAllMedicalComponent(q, (MedicalComponent) b);
						} catch (Exception e) {
							e.printStackTrace();
							return 0;
						}
	...
	public Result<?> getDataCount(MedicalComponent medicalComponent,HttpServletRequest req) throws Exception {
		QueryWrapper<MedicalComponent> queryWrapper = service.getQueryWrapper(medicalComponent, req);
    ...
    @GetMapping(value = "/saveCleanMedicalComponent")
	@RequiresPermissions("basicDataManage:clean")
   	public Result<?> saveCleanMedicalComponent(HttpServletRequest req,MedicalComponent medicalComponent) throws Exception {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		try {
			QueryWrapper<MedicalComponent> queryWrapper = service.getQueryWrapper(medicalComponent, req);
			...
			bean.setActionStaffName(user.getRealname());
			bean.setActionTime(new Date());
//			service.saveCleanMedicalComponent(queryWrapper,bean);
			return importTaskService.saveBatchTask("MEDICAL_COMPONENT","成分表一键清理",bean, queryWrapper,
					(b,q)->{
						 return this.service.saveCleanMedicalComponent(q, (MedicalAuditLog) b);
					});
		} catch (Exception e) {
			log.error(e.getMessage());
			e.printStackTrace();
	...
	@RequiresPermissions("basicDataManage:undoAll")
   	public Result<?> saveUndoAllMedicalAuditLog(HttpServletRequest req,MedicalComponent medicalComponent) throws Exception {
		LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		try {
			QueryWrapper<MedicalComponent> queryWrapper = service.getQueryWrapper(medicalComponent, req);
			//int count = service.count(queryWrapper);
			...
			return importTaskService.saveBatchTask("MEDICAL_COMPONENT","成分表全部撤销",bean, list,
					(b,l)->{
						try {
	...
	public Result<?> saveAuditAllMedicalAuditLog(HttpServletRequest req, MedicalComponent medicalComponent)
			throws Exception {
		...
		try {
			...
			return importTaskService.saveBatchTask("MEDICAL_COMPONENT","成分表批量审核",bean, list,
					(b,l)->{
						try {
	...
	public ModelAndView exportXls(HttpServletRequest request, MedicalComponent medicalComponent) {
		return super.exportXls(request, medicalComponent, MedicalComponent.class, "成分表信息表");
	}
	...
	public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			return importTaskService.saveImportTask("MEDICAL_COMPONENT","成分表信息导入",file,user,
					(f,u)->{
						try {
	...
	public Result<?> exportExcelByThread(HttpServletRequest req, MedicalComponent bean) throws Exception {
		...
		QueryWrapper<MedicalComponent> queryWrapper = service.getQueryWrapper(bean, req);
		int count = service.count(queryWrapper);
	...
	public void exportExcel(HttpServletRequest req,HttpServletResponse response, MedicalComponent bean) throws Exception {
		Result<?> result = new Result<>();
