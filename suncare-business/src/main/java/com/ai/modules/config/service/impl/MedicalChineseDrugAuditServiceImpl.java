...
@Service("chineseDrugAuditService")
@Transactional
public class MedicalChineseDrugAuditServiceImpl implements ICommonAuditService {
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalChineseDrug> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalChineseDrug> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalChineseDrug recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalChineseDrug recordBean,List<MedicalChineseDrug> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalChineseDrug审核数据修改
	private MedicalChineseDrug auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalChineseDrug recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalChineseDrug.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalChineseDrug recordBean = (MedicalChineseDrug)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalChineseDrug  recordBean,List<MedicalChineseDrug> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalChineseDrug(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalChineseDrug(MedicalChineseDrug recordBean, MedicalAuditLog oldBean, List<MedicalChineseDrug> updateList, List<String> deleteList) {
