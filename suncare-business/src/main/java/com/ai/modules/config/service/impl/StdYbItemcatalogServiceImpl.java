...
import com.ai.modules.config.mapper.StdYbItemcatalogMapper;
import com.ai.modules.config.service.IStdYbItemcatalogService;
import com.ai.modules.config.vo.StdYbItemcatalogImport;
 ...
 */
@Service
public class StdYbItemcatalogServiceImpl extends ServiceImpl<StdYbItemcatalogMapper, StdYbItemcatalog> implements IStdYbItemcatalogService {
    @Override
    public boolean isExistName(String code, String id) {
        QueryWrapper<StdYbItemcatalog> queryWrapper = new QueryWrapper<StdYbItemcatalog>();
        queryWrapper.eq("ITEMCODE", code);
        if(StringUtils.isNotBlank(id)){
            queryWrapper.notIn("ID", id);
        }
        List<StdYbItemcatalog> list = this.baseMapper.selectList(queryWrapper);
    ...
    public Result<?> importExcel(MultipartFile file, LoginUser user)throws Exception {
        String mappingFieldStr = "id,itemcode,itemname,chargeclassIdSrc,chargeclassSrc,itemcodeSrc,itemnameSrc,itemContent,exceptContent,chargeunit," +
                "itempriceProvince,itemprice1,itemprice2,itemprice3,itempricecounty1,itempricecounty2,itempricecounty3,fundpayProp," +
                "chargeattriSrc,chargeattricode,chargeattriname,itemnote,parentcode,parentname," +
    ...
    @Override
    public boolean exportExcel(List<StdYbItemcatalog> list, OutputStream os, String suffix) throws Exception {
        SimpleDateFormat date_sdf = new SimpleDateFormat("yyyy-MM-dd");
        String titleStr = "id主键,亚信项目编码,亚信项目名称,财务分类编码(原始),财务分类名称(原始),项目编码(原始),项目名称(原始),项目内涵,除外内容,计价单位," +
                "省级价格(元),市一级价格(元),市二级价格(元),市三级价格(元),县一级价格(元),县二级价格(元),县三级价格(元),首先个人自付比例(%)," +
                "收费项目等级名称(原始),收费项目等级编码(映射后),收费项目等级名称(映射后),项目说明,项目父级编码,项目父级名称," +
        ...
        List<StdYbItemcatalogImport> exportList = new ArrayList<StdYbItemcatalogImport>();
        for (StdYbItemcatalog bean : list) {
            StdYbItemcatalogImport dataBean = new StdYbItemcatalogImport();
        ...
        if (ExcelTool.OFFICE_EXCEL_2010_POSTFIX.equals(suffix)) {
            ...
            ExportXUtils.exportExl(exportList, StdYbItemcatalogImport.class, titles, fields, workbook, "医疗服务项目医保目录");
            workbook.write(os);
            workbook.dispose();
        } else {
            // 创建文件输出流
            WritableWorkbook wwb = Workbook.createWorkbook(os);
            WritableSheet sheet = wwb.createSheet("医疗服务项目医保目录", 0);
            ExportUtils.exportExl(exportList, StdYbItemcatalogImport.class, titles, fields, sheet, "");
            wwb.write();
            wwb.close();
        }
        return false;
    ...
    private Result<?> importExcel(MultipartFile file, LoginUser user, String[] mappingFields) throws Exception, IOException {
        ...
        SimpleDateFormat date_sdf = new SimpleDateFormat("yyyy-MM-dd");
        List<StdYbItemcatalogImport> list = new ArrayList<>();
        String name = file.getOriginalFilename();
        if (name.endsWith(ExcelTool.POINT + ExcelTool.OFFICE_EXCEL_2010_POSTFIX)) {
            list = ExcelXUtils.readSheet(StdYbItemcatalogImport.class, mappingFields, 0, 1, file.getInputStream());
        } else {
            list = ExcelUtils.readSheet(StdYbItemcatalogImport.class, mappingFields, 0, 1, file.getInputStream());
        }
        if (list.size() == 0) {
        ...
        //字典值检验
        List<StdYbItemcatalog> addUpdateList = new ArrayList<StdYbItemcatalog>();
        List<String> deleteList = new ArrayList<String>();//删除id
//        Set<String> codeSet = new HashSet<String>();
        for (int i = 0; i < list.size(); i++) {
            boolean flag = true;
            StdYbItemcatalogImport beanVO = list.get(i);
            ...
            if (StringUtils.isBlank(beanVO.getItemnameSrc())) {
                message += "导入的数据中“项目名称(原始)”不能为空，如：第" + (i + 2) + "行数据“项目名称(原始)”为空\n";
                flag = false;
            }
            ...
            if ("1".equals(beanVO.getImportActionType())) {//新增
                ...
                if(StringUtils.isBlank(beanVO.getId())){
                    ...
                    //判断数据是否存在
                    StdYbItemcatalog oldBean = this.baseMapper.selectById(beanVO.getId());
                    if (oldBean == null) {
            ...
            if (deleteList.size() > 0) {
                ...
                if (idSetList.size() > 0) {
                    for (HashSet<String> idsSet : idSetList) {
                        this.baseMapper.delete(new QueryWrapper<StdYbItemcatalog>().in("ID", idsSet));
                    }
                }
            }
            //批量新增修改
    ...
    private StdYbItemcatalog selectByItemcode(String itemcode){
        QueryWrapper<StdYbItemcatalog> queryWrapper = new QueryWrapper<StdYbItemcatalog>();
