...
@Service("drugPropertyAuditService")
@Transactional
public class MedicalDrugPropertyAuditServiceImpl implements ICommonAuditService {
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalDrugProperty> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalDrugProperty> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalDrugProperty recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalDrugProperty recordBean,List<MedicalDrugProperty> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalDrugProperty审核数据修改
	private MedicalDrugProperty auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalDrugProperty recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalDrugProperty.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalDrugProperty recordBean = (MedicalDrugProperty)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalDrugProperty  recordBean,List<MedicalDrugProperty> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalDrugProperty(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalDrugProperty(MedicalDrugProperty recordBean, MedicalAuditLog oldBean, List<MedicalDrugProperty> updateList, List<String> deleteList) {
