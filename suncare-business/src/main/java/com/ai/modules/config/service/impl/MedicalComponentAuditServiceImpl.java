 ...
 * @Version: V1.0
 */
@Service("componentAuditService")
@Transactional
public class MedicalComponentAuditServiceImpl  implements ICommonAuditService {
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalComponent> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalComponent> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalComponent recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalComponent recordBean,List<MedicalComponent> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalComponent审核数据修改
	private MedicalComponent auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalComponent recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalComponent.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalComponent recordBean = (MedicalComponent)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalComponent  recordBean,List<MedicalComponent> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalComponent(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalComponent(MedicalComponent recordBean,MedicalAuditLog oldBean,List<MedicalComponent> updateList,List<String> deleteList) {
