...
@Service("drugAuditService")
@Transactional
public class MedicalDrugAuditServiceImpl implements ICommonAuditService {
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalDrug> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalDrug> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalDrug recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalDrug recordBean,List<MedicalDrug> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalDrug审核数据修改
	private MedicalDrug auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalDrug recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalDrug.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalDrug recordBean = (MedicalDrug)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalDrug  recordBean,List<MedicalDrug> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalDrug(recordBean,oldBean,updateList,deleteList);

	}

	//撤销操作
	private void undoMedicalDrug(MedicalDrug recordBean,MedicalAuditLog oldBean,List<MedicalDrug> updateList,List<String> deleteList) {
