...
@Service("treatProjectAuditService")
@Transactional
public class MedicalTreatProjectAuditServiceImpl implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalTreatProject> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalTreatProject> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalTreatProject recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalTreatProject recordBean,List<MedicalTreatProject> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalTreatProject审核数据修改
	private MedicalTreatProject auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalTreatProject recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalTreatProject.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalTreatProject recordBean = (MedicalTreatProject)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalTreatProject  recordBean,List<MedicalTreatProject> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalTreatProject(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalTreatProject(MedicalTreatProject recordBean, MedicalAuditLog oldBean, List<MedicalTreatProject> updateList, List<String> deleteList) {
