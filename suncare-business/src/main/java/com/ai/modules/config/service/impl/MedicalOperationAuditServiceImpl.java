...
@Transactional
public class MedicalOperationAuditServiceImpl implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalOperation> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalOperation> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalOperation recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOperation recordBean,List<MedicalOperation> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalOperation审核数据修改
	private MedicalOperation auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalOperation recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalOperation.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalOperation recordBean = (MedicalOperation)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOperation  recordBean,List<MedicalOperation> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalOperation(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalOperation(MedicalOperation recordBean, MedicalAuditLog oldBean, List<MedicalOperation> updateList, List<String> deleteList) {
