...
import com.ai.modules.config.mapper.MedicalComponentMapper;
 ...
 */
@Service
public class MedicalComponentServiceImpl extends ServiceImpl<MedicalComponentMapper, MedicalComponent> implements IMedicalComponentService {
	@Autowired
	IMedicalAuditLogService logService;

	private static final String TABLE_NAME="MEDICAL_COMPONENT";//表名
	...
	public QueryWrapper<MedicalComponent> getQueryWrapper(MedicalComponent medicalComponent,HttpServletRequest request) throws Exception {
		medicalComponent.setState("");
		medicalComponent.setAuditResult("");
		medicalComponent.setActionType("");
		QueryWrapper<MedicalComponent> queryWrapper = QueryGenerator.initQueryWrapper(medicalComponent, request.getParameterMap());
	...
	public List<MedicalComponent> getListByIds(String ids){
		...
		List<MedicalComponent> list = new ArrayList();
	...
	public boolean isExistName(String code, String id) {
		QueryWrapper<MedicalComponent> queryWrapper = new QueryWrapper<MedicalComponent>();
		...
		List<MedicalComponent> list = this.baseMapper.selectList(queryWrapper);
	...
	public void updateMedicalComponent(MedicalComponent bean) {
		MedicalComponent oldBean = this.baseMapper.selectById(bean.getId());
	...
	public void updateBeanBatch(List<MedicalComponent> list, List<MedicalComponent> oldlist) {
		Field[] fields = MedicalComponent.class.getDeclaredFields();
		List<MedicalAuditLog> logAddList = new ArrayList<>();
		for(int i=0;i<list.size();i++){
			MedicalComponent bean = list.get(i);
			MedicalComponent oldBean = oldlist.get(i);
	...
	public void delMedicalComponent(MedicalComponent bean) {
		List<MedicalComponent> list =getListByIds(bean.getId());
		commonDelMedicalComponentBatch(bean, list);
	}

	//删除单条操作
	private void commonDelMedicalComponentBatch(MedicalComponent bean, List<MedicalComponent> oldlist) {
		List<MedicalAuditLog> logAddList = new ArrayList<>();
		for (MedicalComponent oldBean : oldlist) {
	...
	@Override
	@Transactional
	public int delAllMedicalComponent(QueryWrapper<MedicalComponent> queryWrapper, MedicalComponent bean)
			throws Exception {
		List<MedicalComponent> list = this.baseMapper.selectList(queryWrapper);
		commonDelMedicalComponentBatch(bean, list);
		return list.size();
	...
	public Result<?> importExcel(MultipartFile file,LoginUser user) throws Exception {
		String mappingFieldStr = "code,name,ename,otherName,toxicityCode,toxicity,referencetypeCode,referencetype,sourceCode,source,remark,createReason,actionType";//导入的字段
	...
	private Result<?> allImportExcel(MultipartFile file, LoginUser user,String[] mappingFields) throws Exception, IOException {
		...
		List<MedicalComponent> list = new ArrayList<MedicalComponent>();
		String name = file.getOriginalFilename();
		if (name.endsWith(ExcelTool.POINT+ExcelTool.OFFICE_EXCEL_2010_POSTFIX)) {
			list = ExcelXUtils.readSheet(MedicalComponent.class, mappingFields, 0, 1, file.getInputStream());
		}else {
			list = ExcelUtils.readSheet(MedicalComponent.class, mappingFields, 0, 1, file.getInputStream());
		}
		if(list.size() == 0) {
		...
		List<MedicalComponent> addList = new ArrayList<MedicalComponent>();
		List<MedicalComponent> olnyUpdateList = new ArrayList<MedicalComponent>();
		List<MedicalComponent> updateList = new ArrayList<MedicalComponent>();
		List<MedicalComponent> oldUpdateList = new ArrayList<MedicalComponent>();
		List<MedicalComponent> deleteList = new ArrayList<MedicalComponent>();
		...
		Map<String,MedicalComponent> addMap = new HashMap<>();
		Map<String,MedicalComponent> updateMap = new HashMap<>();
		Map<String,MedicalComponent> deleteMap = new HashMap<>();
		List<String> errorMsg =  new ArrayList<>();
		for (int i = 0; i < list.size(); i++) {
			MedicalComponent bean = list.get(i);
			if (StringUtils.isBlank(bean.getCode())) {
				errorMsg.add("导入的数据中“成分编码”不能为空，如：第" + (i + 2) + "行数据“成分编码”为空");
			}
			if (StringUtils.isBlank(bean.getName())) {
				errorMsg.add("导入的数据中“成分名称”不能为空，如：第" + (i + 2) + "行数据“成分名称”为空");
			}
			//判断code在excel中是否重复
			if(codeSet.contains(bean.getCode())){
				errorMsg.add("导入的数据中“成分编码”不能重复，如：第" + (i + 2) + "行数据“"+bean.getCode()+"”在excel中重复");
			}
		...
		if(codesUpdate.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalComponent> existList = getBeanByCode(codesUpdate,stateList,null);
			List<String> existCode = existList.stream().map(MedicalComponent::getCode).collect(Collectors.toList());
			...
			if(errorMsg.size()==0) {
				existList.forEach(oldBean -> {
					MedicalComponent bean = updateMap.get(oldBean.getCode());
		...
		if(codesDelete.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalComponent> existList = getBeanByCode(codesDelete,stateList,null);
			List<String> existCode = existList.stream().map(MedicalComponent::getCode).collect(Collectors.toList());
			...
			if(errorMsg.size()==0) {
				existList.forEach(oldBean -> {
					MedicalComponent bean = deleteMap.get(oldBean.getCode());
					bean.setId(oldBean.getId());
	...
	private List<MedicalComponent> getBeanByCode(List<String> codes,List<String> stateList,String[] fileds){
		List<MedicalComponent> alllist = new ArrayList<>();
	...
	private MedicalComponent findBeanByCode(String code,String state) {
		QueryWrapper<MedicalComponent> queryWrapper = new QueryWrapper<MedicalComponent>();
		queryWrapper.eq("CODE", code);
	...
	public boolean exportExcel(QueryWrapper<MedicalComponent> queryWrapper, OutputStream os, String suffix){
		boolean isSuc = true;
		try {
			List<MedicalComponent> list = this.list(queryWrapper);
			List<MedicalComponent> dataList = new ArrayList<MedicalComponent>();
	    	Map<String,List<Map<String,Object>>> typeMap = new HashMap<String,List<Map<String,Object>>>();
	    	for(MedicalComponent exportBean:list){
	    	...
	    	String titleStr = "成分编码,成分名称,成分英文名称,成分异名,毒性程度编码,毒性程度,参考类型编码,参考类型名称,参考来源编码,参考来源名称,备注,"
	    			+ "最近一次操作类型,数据状态,审核状态,审核人,审核时间,审核意见,"
	    			+ "新增人,新增时间,新增原因,最新修改人,最新修改时间,修改原因,删除人,删除时间,删除原因";
	    	String[] titles= titleStr.split(",");
	    	String fieldStr = "code,name,ename,otherName,toxicityCode,toxicity,referencetypeCode,referencetype,sourceCode,source,remark,"
			...
			if("xlsx".equals(suffix)) {
				ExcelUtils.writeOneSheetSXSSFWorkbook(dataList,titles,fields,"数据",os);
			}else {
				ExcelUtils.writeOneSheet(dataList,titles,fields,"数据",os);
			}
    	} catch (Exception e) {
			e.printStackTrace();
			isSuc = false;
