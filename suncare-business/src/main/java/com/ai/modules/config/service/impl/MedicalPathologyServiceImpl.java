...
import com.ai.modules.config.mapper.MedicalPathologyMapper;
 ...
 */
@Service
public class MedicalPathologyServiceImpl extends ServiceImpl<MedicalPathologyMapper, MedicalPathology> implements IMedicalPathologyService {
	...
	private static final String TABLE_NAME="MEDICAL_PATHOLOGY";//表名

	private static final String DICT_ENAME="pathologyclass";//字典名称
	...
	public QueryWrapper<MedicalPathology> getQueryWrapper(MedicalPathology medicalPathology,HttpServletRequest request) throws Exception {
		medicalPathology.setState("");
		medicalPathology.setAuditResult("");
		medicalPathology.setActionType("");
		QueryWrapper<MedicalPathology> queryWrapper = QueryGenerator.initQueryWrapper(medicalPathology, request.getParameterMap());
	...
	public List<MedicalPathology> getListByIds(String ids){
		...
		List<MedicalPathology> list = new ArrayList();
	...
	private void setTypeCodes(MedicalPathology bean,Map<String,List<Map<String,Object>>> typeMap,List<Map<String,Object>> typeList) {
		String typeCode = bean.getParentCode();
	...
	public boolean isExistName(String code, String id) {
		QueryWrapper<MedicalPathology> queryWrapper = new QueryWrapper<MedicalPathology>();
		...
		List<MedicalPathology> list = this.baseMapper.selectList(queryWrapper);
	...
	public void updateMedicalPathology(MedicalPathology bean) {
		setTypeCodes(bean,null,null);//设置各级分类编码和分类名称
		MedicalPathology oldBean = this.baseMapper.selectById(bean.getId());
...
//				mapper.updateByPrimaryKeySelective(oldBean);
	...
	public void updateBeanBatch(List<MedicalPathology> list, List<MedicalPathology> oldlist) {
		Field[] fields = MedicalPathology.class.getDeclaredFields();
		List<MedicalAuditLog> logAddList = new ArrayList<>();
		for(int i=0;i<list.size();i++){
			MedicalPathology bean = list.get(i);
			MedicalPathology oldBean = oldlist.get(i);
	...
	public void delMedicalPathology(MedicalPathology bean) {
		List<MedicalPathology> list =getListByIds(bean.getId());
		commonDelMedicalPathologyBatch(bean, list);
	}

	//删除单条操作
	private void commonDelMedicalPathologyBatch(MedicalPathology bean, List<MedicalPathology> oldlist) {
		List<MedicalAuditLog> logAddList = new ArrayList<>();
		for (MedicalPathology oldBean : oldlist) {
	...
	@Override
	@Transactional
	public int delAllMedicalPathology(QueryWrapper<MedicalPathology> queryWrapper, MedicalPathology bean)
			throws Exception {
		List<MedicalPathology> list = this.baseMapper.selectList(queryWrapper);
		commonDelMedicalPathologyBatch(bean, list);
		return list.size();
	...
	public Result<?> importExcel(MultipartFile file,LoginUser user) throws Exception {
		String mappingFieldStr = "code,name,parentCode,parentName,descript,createReason,actionType";//导入的字段
	...
	private Result<?> allImportExcel(MultipartFile file, LoginUser user,String[] mappingFields) throws Exception, IOException {
		...
		List<MedicalPathology> list = new ArrayList<MedicalPathology>();
		String name = file.getOriginalFilename();
		if (name.endsWith(ExcelTool.POINT+ExcelTool.OFFICE_EXCEL_2010_POSTFIX)) {
			list = ExcelXUtils.readSheet(MedicalPathology.class, mappingFields, 0, 1, file.getInputStream());
		}else {
			list = ExcelUtils.readSheet(MedicalPathology.class, mappingFields, 0, 1, file.getInputStream());
		}
		if(list.size() == 0) {
		...
		List<MedicalPathology> addList = new ArrayList<MedicalPathology>();
		List<MedicalPathology> olnyUpdateList = new ArrayList<MedicalPathology>();
		List<MedicalPathology> updateList = new ArrayList<MedicalPathology>();
		List<MedicalPathology> oldUpdateList = new ArrayList<MedicalPathology>();
		List<MedicalPathology> deleteList = new ArrayList<MedicalPathology>();
		...
		Map<String,MedicalPathology> addMap = new HashMap<>();
		Map<String,MedicalPathology> updateMap = new HashMap<>();
		Map<String,MedicalPathology> deleteMap = new HashMap<>();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (StringUtils.isBlank(bean.getCode())) {
				errorMsg.add("导入的数据中“YX形态学编码”不能为空，如：第" + (i + 2) + "行数据“YX形态学编码”为空");
			}
			if (StringUtils.isBlank(bean.getName())) {
				errorMsg.add("导入的数据中“YX形态学名称”不能为空，如：第" + (i + 2) + "行数据“YX形态学名称”为空");
			}
			...
			if (StringUtils.isBlank(bean.getParentName())) {
				errorMsg.add("导入的数据中“父级分类名称”不能为空，如：第" + (i + 2) + "行数据“父级分类名称”为空");
			}
			//判断code在excel中是否重复
			if(codeSet.contains(bean.getCode())){
				errorMsg.add("导入的数据中“YX形态学编码”不能重复，如：第" + (i + 2) + "行数据“"+bean.getCode()+"”在excel中重复");
			}
		...
		if(codesAdd.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalPathology> existList = getBeanByCode(codesAdd,stateList,"CODE".split(","));
			List<String> existCode = existList.stream().map(MedicalPathology::getCode).collect(Collectors.toList());
			if(existCode.size()>0){
		...
		if(codesUpdate.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalPathology> existList = getBeanByCode(codesUpdate,stateList,null);
			List<String> existCode = existList.stream().map(MedicalPathology::getCode).collect(Collectors.toList());
			...
			if(errorMsg.size()==0) {
				existList.forEach(oldBean -> {
					MedicalPathology bean = updateMap.get(oldBean.getCode());
		...
		if(codesDelete.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalPathology> existList = getBeanByCode(codesDelete,stateList,null);
			List<String> existCode = existList.stream().map(MedicalPathology::getCode).collect(Collectors.toList());
			...
			if(errorMsg.size()==0) {
				existList.forEach(oldBean -> {
					MedicalPathology bean = deleteMap.get(oldBean.getCode());
					bean.setId(oldBean.getId());
	...
	private List<MedicalPathology> getBeanByCode(List<String> codes,List<String> stateList,String[] fileds){
		List<MedicalPathology> alllist = new ArrayList<>();
	...
	private MedicalPathology findBeanByCode(String code,String state) {
		QueryWrapper<MedicalPathology> queryWrapper = new QueryWrapper<MedicalPathology>();
		queryWrapper.eq("CODE", code);
	...
	public boolean exportExcel(QueryWrapper<MedicalPathology> queryWrapper, OutputStream os, String suffix){
		boolean isSuc = true;
		try {
			List<MedicalPathology> list = this.list(queryWrapper);
			List<MedicalPathology> dataList = new ArrayList<MedicalPathology>();
	    	Map<String,List<Map<String,Object>>> typeMap = new HashMap<String,List<Map<String,Object>>>();
	    	for(MedicalPathology exportBean:list){
	    	...
	    	String titleStr = "YX形态学编码,YX形态学名称,父级分类编码,父级分类名称,描述,"
	    			+ "最近一次操作类型,数据状态,审核状态,审核人,审核时间,审核意见,"
	    			+ "新增人,新增时间,新增原因,最新修改人,最新修改时间,修改原因,删除人,删除时间,删除原因";
	    	String[] titles= titleStr.split(",");
	    	String fieldStr = "code,name,parentCode,parentName,descript,"
			...
			if(ExcelTool.OFFICE_EXCEL_2010_POSTFIX.equals(suffix)) {
			    ...
			    ExportXUtils.exportExl(dataList,MedicalPathology.class,titles,fields,workbook,"数据");
			    workbook.write(os);
		        workbook.dispose();
			}else {
				 // 创建文件输出流
				...
				ExportUtils.exportExl(dataList,MedicalPathology.class,titles,fields,sheet, "");
				wwb.write();
		        wwb.close();
			}
    	} catch (Exception e) {
