...
@Service("otherDictAuditService")
@Transactional
public class MedicalOtherDictAuditServiceImpl implements ICommonAuditService {
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalOtherDict> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalOtherDict> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalOtherDict recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOtherDict recordBean,List<MedicalOtherDict> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalOtherDict审核数据修改
	private MedicalOtherDict auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalOtherDict recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalOtherDict.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalOtherDict recordBean = (MedicalOtherDict)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOtherDict  recordBean,List<MedicalOtherDict> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalOtherDict(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalOtherDict(MedicalOtherDict recordBean, MedicalAuditLog oldBean, List<MedicalOtherDict> updateList, List<String> deleteList) {
