...
@Transactional
public class MedicalOfficeAuditServiceImpl implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalOffice> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalOffice> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalOffice recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOffice recordBean,List<MedicalOffice> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalOffice审核数据修改
	private MedicalOffice auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalOffice recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalOffice.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalOffice recordBean = (MedicalOffice)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOffice  recordBean,List<MedicalOffice> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalOffice(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalOffice(MedicalOffice recordBean, MedicalAuditLog oldBean, List<MedicalOffice> updateList, List<String> deleteList) {
