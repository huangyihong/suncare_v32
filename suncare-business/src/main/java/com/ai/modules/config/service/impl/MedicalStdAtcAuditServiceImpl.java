...
@Service("stdAtcAuditService")
@Transactional
public class MedicalStdAtcAuditServiceImpl implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalStdAtc> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalStdAtc> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalStdAtc recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalStdAtc recordBean,List<MedicalStdAtc> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalStdAtc审核数据修改
	private MedicalStdAtc auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalStdAtc recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalStdAtc.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalStdAtc recordBean = (MedicalStdAtc)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalStdAtc  recordBean,List<MedicalStdAtc> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalStdAtc(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalStdAtc(MedicalStdAtc recordBean, MedicalAuditLog oldBean, List<MedicalStdAtc> updateList, List<String> deleteList) {
