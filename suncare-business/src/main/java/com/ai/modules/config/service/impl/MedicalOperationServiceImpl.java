...
import com.ai.modules.config.mapper.MedicalOperationMapper;
...
import com.ai.modules.config.vo.MedicalOperationExport;
 ...
 */
@Service
public class MedicalOperationServiceImpl extends ServiceImpl<MedicalOperationMapper, MedicalOperation> implements IMedicalOperationService {
	@Autowired
	...
	private static final String TABLE_NAME="MEDICAL_OPERATION";//表名

	private static final String DICT_ENAME="STD_ICD_surgery";
	...
	public QueryWrapper<MedicalOperation> getQueryWrapper(MedicalOperation medicalOperation,HttpServletRequest request) throws Exception {
		medicalOperation.setState("");
		medicalOperation.setAuditResult("");
		medicalOperation.setActionType("");
		QueryWrapper<MedicalOperation> queryWrapper = QueryGenerator.initQueryWrapper(medicalOperation, request.getParameterMap());
	...
	public List<MedicalOperation> getListByIds(String ids){
		...
		List<MedicalOperation> list = new ArrayList();
	...
	private void setTypeCodes(MedicalOperation bean, Map<String,List<Map<String,Object>>> typeMap, List<Map<String,Object>> typeList) {
	...
	public boolean isExistName(String code, String id) {
		QueryWrapper<MedicalOperation> queryWrapper = new QueryWrapper<MedicalOperation>();
		...
		List<MedicalOperation> list = this.baseMapper.selectList(queryWrapper);
	...
	public void updateMedicalOperation(MedicalOperation bean) {
		MedicalOperation oldBean = this.baseMapper.selectById(bean.getId());
	...
	public void updateBeanBatch(List<MedicalOperation> list, List<MedicalOperation> oldlist) {
		Field[] fields = MedicalOperation.class.getDeclaredFields();
		List<MedicalAuditLog> logAddList = new ArrayList<>();
		for(int i=0;i<list.size();i++){
			MedicalOperation bean = list.get(i);
			MedicalOperation oldBean = oldlist.get(i);
	...
	public void delMedicalOperation(MedicalOperation bean) {
		List<MedicalOperation> list =getListByIds(bean.getId());
		commonDelMedicalOperationBatch(bean, list);
	}

	//删除单条操作
	private void commonDelMedicalOperationBatch(MedicalOperation bean, List<MedicalOperation> oldlist) {
		List<MedicalAuditLog> logAddList = new ArrayList<>();
		for (MedicalOperation oldBean : oldlist) {
	...
	@Override
	@Transactional
	public int delAllMedicalOperation(QueryWrapper<MedicalOperation> queryWrapper, MedicalOperation bean)
			throws Exception {
		List<MedicalOperation> list = this.baseMapper.selectList(queryWrapper);
		commonDelMedicalOperationBatch(bean, list);
		return list.size();
	...
	public Result<?> importExcel(MultipartFile file,LoginUser user) throws Exception {
		String mappingFieldStr = "typeCode,typeName,code,name,operationLevelCode,operationLevel,bacterialLevel,bacterialLevelName,remark,createReason,actionType";//导入的字段
	...
	private Result<?> allImportExcel(MultipartFile file, LoginUser user,String[] mappingFields) throws Exception, IOException {
		...
		List<MedicalOperation> list = new ArrayList<MedicalOperation>();
		String filename = file.getOriginalFilename();
		if (filename.endsWith(ExcelTool.POINT+ExcelTool.OFFICE_EXCEL_2010_POSTFIX)) {
			list = ExcelXUtils.readSheet(MedicalOperation.class, mappingFields, 0, 1, file.getInputStream());
		}else {
			list = ExcelUtils.readSheet(MedicalOperation.class, mappingFields, 0, 1, file.getInputStream());
		}
		if(list.size() == 0) {
		...
		List<MedicalOperation> addList = new ArrayList<MedicalOperation>();
		List<MedicalOperation> olnyUpdateList = new ArrayList<MedicalOperation>();
		List<MedicalOperation> updateList = new ArrayList<MedicalOperation>();
		List<MedicalOperation> oldUpdateList = new ArrayList<MedicalOperation>();
		List<MedicalOperation> deleteList = new ArrayList<MedicalOperation>();
		...
		Map<String,MedicalOperation> addMap = new HashMap<>();
		Map<String,MedicalOperation> updateMap = new HashMap<>();
		Map<String,MedicalOperation> deleteMap = new HashMap<>();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (StringUtils.isBlank(bean.getTypeCode())) {
				errorMsg.add("导入的数据中“上级分类编码”不能为空，如：第" + (i + 2) + "行数据“上级分类编码”为空");
			}
			if (StringUtils.isBlank(bean.getCode())) {
				errorMsg.add("导入的数据中“手术及操作编码”不能为空，如：第" + (i + 2) + "行数据“手术及操作编码”为空");
			}
			if (StringUtils.isBlank(bean.getName())) {
				errorMsg.add("导入的数据中“手术及操作名称”不能为空，如：第" + (i + 2) + "行数据“手术及操作名称”为空");
			}
			//判断code在excel中是否重复
			if(codeSet.contains(bean.getCode())){
				errorMsg.add("导入的数据中“手术及操作编码”不能重复，如：第" + (i + 2) + "行数据“"+bean.getCode()+"”在excel中重复");
			}
		...
		if(codesAdd.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalOperation> existList = getBeanByCode(codesAdd,stateList,"CODE".split(","));
			List<String> existCode = existList.stream().map(MedicalOperation::getCode).collect(Collectors.toList());
			if(existCode.size()>0){
		...
		if(codesUpdate.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalOperation> existList = getBeanByCode(codesUpdate,stateList,null);
			List<String> existCode = existList.stream().map(MedicalOperation::getCode).collect(Collectors.toList());
			...
			if(errorMsg.size()==0) {
				existList.forEach(oldBean -> {
					MedicalOperation bean = updateMap.get(oldBean.getCode());
		...
		if(codesDelete.size()>0){
			...
			stateList.add(MedicalAuditLogConstants.STATE_DSX);
			stateList.add(MedicalAuditLogConstants.STATE_YX);
			List<MedicalOperation> existList = getBeanByCode(codesDelete,stateList,null);
			List<String> existCode = existList.stream().map(MedicalOperation::getCode).collect(Collectors.toList());
			...
			if(errorMsg.size()==0) {
				existList.forEach(oldBean -> {
					MedicalOperation bean = deleteMap.get(oldBean.getCode());
					bean.setId(oldBean.getId());
	...
	private List<MedicalOperation> getBeanByCode(List<String> codes,List<String> stateList,String[] fileds){
		List<MedicalOperation> alllist = new ArrayList<>();
	...
	private MedicalOperation findBeanByCode(String code,String state) {
		QueryWrapper<MedicalOperation> queryWrapper = new QueryWrapper<MedicalOperation>();
		queryWrapper.eq("CODE", code);
	...
	public boolean exportExcel(QueryWrapper<MedicalOperation> queryWrapper, OutputStream os, String suffix){
		boolean isSuc = true;
		try {
			List<MedicalOperation> list = this.list(queryWrapper);
			List<MedicalOperationExport> dataList = new ArrayList<MedicalOperationExport>();
	    	Map<String,List<Map<String,Object>>> typeMap = new HashMap<String,List<Map<String,Object>>>();
	    	for(MedicalOperation bean:list){
				String typeCode = bean.getTypeCode();
				List<Map<String,Object>> typeList = typeMap.get(typeCode);
				this.setTypeCodes(bean,typeMap,typeList);
	    		MedicalOperationExport exportBean = new MedicalOperationExport();
	    	...
	    	String titleStr = "章编码,章名称,节编码,节名称,类目编码,类目名称,亚目编码,亚目名称,"
	    			+ "手术及操作编码,手术及操作名称,手术级别编码,手术级别名称,手术无菌程度代码,手术无菌程度名称,备注,最近一次操作类型,数据状态,审核状态,审核人,审核时间,审核意见,"
	    			+ "新增人,新增时间,新增原因,最新修改人,最新修改时间,修改原因,删除人,删除时间,删除原因";
	    	String[] titles= titleStr.split(",");
	    	String fieldStr = "typeCode1,typeName1,typeCode2,typeName2,typeCode3,typeName3,typeCode4,typeName4,"
	    			+ "code,name,operationLevelCode,operationLevel,bacterialLevel,bacterialLevelName,remark,actionType,state,auditResult,auditStaffName,auditTime,auditOpinion,"
			...
			if(ExcelTool.OFFICE_EXCEL_2010_POSTFIX.equals(suffix)) {
			    ...
			    ExportXUtils.exportExl(dataList,MedicalOperationExport.class,titles,fields,workbook,"数据");
			    workbook.write(os);
		        workbook.dispose();
			}else {
				 // 创建文件输出流
				...
				ExportUtils.exportExl(dataList,MedicalOperationExport.class,titles,fields,sheet, "");
				wwb.write();
		        wwb.close();
			}
    	} catch (Exception e) {
