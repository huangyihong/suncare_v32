...
@Service("organAuditService")
@Transactional
public class MedicalOrganAuditServiceImpl implements ICommonAuditService {
	@Autowired
	...
	public void saveAuditMedicalAuditLog(MedicalAuditLog bean) {
		List<MedicalOrgan> updateList = new ArrayList<>();
		List<MedicalAuditLog> logUpdateList = new ArrayList<>();
		String tableName = bean.getTableName();
		List<MedicalOrgan> list =  this.service.getListByIds(bean.getRecordKey());
		for(MedicalOrgan recordBean:list){
	...
	//审核单条操作
	private void commonAuditMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOrgan recordBean,List<MedicalOrgan> updateList,List<MedicalAuditLog> logUpdateList) {
	...
	//MedicalOrgan审核数据修改
	private MedicalOrgan auditBean(MedicalAuditLog oldBean, MedicalAuditLog bean, MedicalOrgan recordBean) {
		if(MedicalAuditLogConstants.ACTIONTYPE_ADD.equals(oldBean.getActionType())){//新增
			...
			if(MedicalAuditLogConstants.AUDITRESULT_SHTG.equals(bean.getAuditResult())){//审核通过
				if(StringUtils.isNotBlank(oldBean.getUpdateJson())){
					recordBean = JSON.parseObject(oldBean.getUpdateJson(), MedicalOrgan.class);//修改的对象
	...
	private void saveUndoMedicalAuditLog(MedicalAuditLog bean, List<Object> list) {
		...
		String tableName = bean.getTableName();
		for(Object obj:list){
			MedicalOrgan recordBean = (MedicalOrgan)obj;
	...
	//撤回单条操作
	private void commonUndoMedicalAuditLog(MedicalAuditLog bean, String tableName,MedicalOrgan  recordBean,List<MedicalOrgan> updateList,List<String> deleteList,List<MedicalAuditLog> logAddList,List<String> logDeleteList) {
		List<MedicalAuditLog> logList = serviceLog.getMedicalAuditLogListByKey(recordBean.getId(),tableName,null);
		MedicalAuditLog oldBean = new MedicalAuditLog();
		if(logList.size()>0){
			oldBean = logList.get(0);
		...
		this.undoMedicalOrgan(recordBean,oldBean,updateList,deleteList);
	}

	//撤销操作
	private void undoMedicalOrgan(MedicalOrgan recordBean, MedicalAuditLog oldBean, List<MedicalOrgan> updateList, List<String> deleteList) {
