...
import com.ai.modules.engine.runnable.EngineChargeRunnable;
...
import com.ai.modules.engine.runnable.EngineTrailChargeRunnable;
...
@Service
@Slf4j
public class EngineChargeServiceImpl implements IEngineChargeService {
	@Autowired
	...
	@Override
	public void generateMedicalUnreasonableChargeAction(TaskProject task, TaskProjectBatch batch, String itemCode, List<MedicalDrugRule> ruleList) throws Exception {
		...
		try {
            ...
            taskSV.updateTaskBatchBreakRuleLog(batch.getBatchId(), "charge", itemCode, entity);
			...
			for(int i=0, len=ruleList.size(); i<len; i++) {
				...
				List<String> tempList = this.parseChargeRuleCondition(ruleEntity);
				if(!tempList.isEmpty()) {
					sb.append("(");
			...
			//一个收费项目只有一条规则且限定范围唯一时，直接沉淀限定范围无需再次计算
	        ...
	        //导入solr
	      	if(count>0) {
	      		...
	      		//计算违规限定范围
	      		if(limitScopeEnum==null || limitScopeEnum.equals(EngineLimitScopeEnum.CODE_38)) {
	      			for(MedicalDrugRule rule : ruleList) {
	          	...
	          	//同步数据
				engineActionService.syncUnreasonableAction(task, batch, itemCode, "2", true);
				//不合规行为汇总
	            engineActionService.executeGroupBy(batch.getBatchId(), itemCode, MedicalConstant.ENGINE_BUSI_TYPE_CHARGE);
	      	}
		} catch(Exception e) {
			success = false;
			error = e.getMessage();
	...
	@Override
	public void generateMedicalUnreasonableChargeActionByThreadPool(String batchId){
	...
	public void generateUnreasonableAction(String batchId) {
		...
		try {
			...
			//先导出已审核的不合规行为结果
			EngineCntResult cntResult = engineActionService.importApprovalAction(batchId, MedicalConstant.ENGINE_BUSI_TYPE_CHARGE);
			taskSV.removeTaskBatchBreakRuleLog(batchId, "charge");
			//删除历史solr数据
			engineActionService.deleteSolr(batchId, "2", true);

			List<MedicalDrugRule> drugRuleList = queryBatchRuleSV.queryMedicalDrugRule(batchId, MedicalConstant.RULE_TYPE_CHARGE);
			if(drugRuleList!=null && drugRuleList.size()>0) {
				...
				//开启线程池跑任务
				List<EngineChargeRunnable> runnables = new ArrayList<EngineChargeRunnable>();
				// 有界队列
				...
				//遍历收费项目
				for(Map.Entry<String, List<MedicalDrugRule>> entry : drugRuleGroupMap.entrySet()) {
					EngineChargeRunnable runnable = new EngineChargeRunnable(datasource, true, task, batch, entry.getKey(), entry.getValue());
				...
				for(EngineChargeRunnable runnable : runnables) {
		...
		log.info(batchId+"批次收费项目总耗时:" + df.format(new Date((long)rtimer.getTime())));
	}
	...
	public void generateUnreasonableAction(String batchId, String itemCode) {
		...
		try {
			...
			//先导出已审核的不合规行为结果
			EngineCntResult cntResult = engineActionService.importApprovalAction(batchId, MedicalConstant.ENGINE_BUSI_TYPE_CHARGE, itemCode);
			//删除历史solr数据
			engineActionService.deleteSolr(batchId, itemCode, "2", true);

			List<MedicalDrugRule> ruleList = queryBatchRuleSV.queryMedicalDrugRuleByItem(batchId, MedicalConstant.RULE_TYPE_CHARGE, itemCode);
			if(ruleList!=null && ruleList.size()>0) {
				this.generateMedicalUnreasonableChargeAction(task, batch, itemCode, ruleList);
			}
			if(cntResult.isSuccess() && cntResult.getCount()>0) {
    			//最后回填已审核的不合规行为结果
            ...
            if (!success) {
            	...
            	Map<String, Integer> statusMap = taskSV.groupByTaskBatchBreakRuleLog(batchId, "charge");
	...
	public void generateUnreasonableActionByThreadPool(String batchId, String itemCode) {
		...
		TaskBatchBreakRuleLog log = new TaskBatchBreakRuleLog();
        log.setStatus(MedicalConstant.RUN_STATE_WAIT);
        taskSV.updateTaskBatchBreakRuleLog(batchId, "charge", itemCode, log);
		String datasource = SolrUtil.getLoginUserDatasource();
    ...
    public void generateUnreasonableActionFailRerun(String batchId) {
		List<MedicalDrugRule> ruleList = queryBatchRuleSV.queryMedicalDrugRuleFail(batchId, MedicalConstant.RULE_TYPE_CHARGE);
        if (ruleList == null || ruleList.size() == 0) {
        	return;
        }
    	TaskBatchStepItem entity = new TaskBatchStepItem();
	    ...
	    for(List<String> sublist : mglist) {
	    	taskSV.updateTaskBatchBreakRuleLog(batchId, "charge", sublist, log);
	    }
	...
	private List<String> parseChargeRuleCondition(EngineDrugRuleEntity ruleEntity) {
	...
	@Override
    public void trailChargeActionThreadPool(String ruleId, String etlSource) {
        ...
        step.setStep(1);
        step.setItemId(MedicalConstant.RULE_TYPE_CHARGE);
        step.setCreateTime(now);
        step.setStatus(status);
		step.setDataSource(ds);
        taskSV.saveTaskBatchStepItem(step);

        ThreadUtils.THREAD_TRAIL_POOL.add(new EngineTrailChargeRunnable(ds, ruleId, etlSource));
    }

	@Override
    public void trailChargeAction(String ruleId, String etlSource, String datasource) {
		String batchId = ruleId;
        boolean success = true;
        String error = null;
        try {
            //删除历史数据
        	String where = "RULE_TYPE:2 AND BATCH_ID:%s";
        	where = String.format(where, batchId);
            SolrUtil.delete(EngineUtil.MEDICAL_TRAIL_DRUG_ACTION, where);
            where = "BUSI_TYPE:CHARGE AND BATCH_ID:%s";
        	where = String.format(where, batchId);
            ...
            taskSV.updateTaskBatchStepItem(batchId, MedicalConstant.RULE_TYPE_CHARGE, datasource, entity);
            ...
            for (MedicalDrugRule rule : drugRuleList) {
                try {
                    result = this.trailChargeAction(rule, etlSource);
                } catch (Exception e) {
                    log.error("", e);
                    success = false;
                    error = error + "\n" + e.getMessage();
            ...
            //试算结果数据同步
            engineActionService.syncTrailAction(ruleId, "2");
        } catch (Exception e) {
        	log.info("", e);
            error = e.getMessage();
            success = false;
	...
	private EngineDrugRuleEntity trailChargeAction(MedicalDrugRule rule, String etlSource) throws Exception {
        ...
        List<String> conditionList = new ArrayList<String>();
        StringBuilder sb = new StringBuilder();
        List<String> tempList = this.parseChargeRuleCondition(result);
        if (!tempList.isEmpty()) {
            sb.append("(");
            for (int j = 0; j < tempList.size(); j++) {
                String condition = tempList.get(j);
    ...
    private ActionTypeDTO getActionTypeDTO() {
    	String busiType = MedicalConstant.ENGINE_BUSI_TYPE_CHARGE;
