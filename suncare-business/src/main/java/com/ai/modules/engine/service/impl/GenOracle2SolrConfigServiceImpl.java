...
@Transactional
@Slf4j
public class GenOracle2SolrConfigServiceImpl implements GenOracle2SolrConfigService {
	...
	public void genSolrConfig(String name, String pk) {
		...
		try {
			...
			String sql = "select t.TABLE_NAME, t.COLUMN_NAME, t.DATA_TYPE, t.DATA_LENGTH, t.DATA_PRECISION,t.DATA_SCALE, t1.COMMENTS from user_tab_columns t,user_col_comments t1 where t.Table_Name= '" + name +  "' and t.Table_Name = t1.Table_Name and t.column_name = t1.column_name";
			log.info(sql);
			...
			while (rs.next()) {
				String col_name = rs.getString("COLUMN_NAME").toUpperCase();
				String data_type = rs.getString("DATA_TYPE");
				String comment = rs.getString("COMMENTS");
				if("DATE".equalsIgnoreCase(data_type)) {
//					importSb.append("\n\t\t").append("to_char(").append(col_name).append(", 'yyyy-MM-dd hh24:mi:ss'").append(") ").append(col_name).append(",");
					importSb.append("\n\t\t").append("date_format(").append(col_name).append(", '%Y-%m-%d %H:%i:%S'").append(") ").append(col_name).append(",");
				} else {
