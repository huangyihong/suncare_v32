 ...
 */
public class SolrOverFreqRuleHandle extends SolrRuleHandle {
	
	public SolrOverFreqRuleHandle(TaskProject task, TaskProjectBatch batch, Boolean trail, MedicalRuleConfig rule,
	...
	public void generateUnreasonableAction() throws Exception {
		...
		if(sync) {
			//数仓hive模式
			SyncUtil.syncChargeFreq(task.getDataSource());
		}
        ...
        //写文件头
        fileWriter.write("[");
        String collection = sync ? "MEDICAL_PATIENT_1VISIT_1DAY_ITEMSUM_CHARGE_FREQ" : "DWS_PATIENT_1VISIT_1DAY_ITEMSUM";
