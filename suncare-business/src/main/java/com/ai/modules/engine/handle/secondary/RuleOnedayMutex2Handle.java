 ...
 */
public class RuleOnedayMutex2Handle extends AbsRuleSecondHandle {

	public RuleOnedayMutex2Handle(TaskProject task, TaskProjectBatch batch, MedicalRuleConfig rule, List<MedicalRuleConditionSet> ruleConditionList, Boolean trail) {
	...
	public void execute() throws Exception {		
    	...
    	String codes = rule.getItemCodes(); //规则主体项目
  		...
  		for(String itemcode : itemcodeList) {
  			...
  			String dws = "TREAT".equals(rule.getRuleType()) ? "MEDICAL_PATIENT_1VISIT_1DAY_ITEMSUM_TREAT_ONEDAYMUTEX" : "MEDICAL_PATIENT_1VISIT_1DAY_ITEMSUM";
  			if(!sync) {
  	    	...
  	    	if(count>0) {
  			    ...
  			    for(Map.Entry<String, OnedayMutexComputeVO> entry : computeMap.entrySet()) {
  	    	    	...
  	    	    	List<SolrDocument> documentList = vo.computeMutex("BREAK_RULE_TIME");
