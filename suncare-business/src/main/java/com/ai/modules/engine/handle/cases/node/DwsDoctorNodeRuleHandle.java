 ...
 */
public class DwsDoctorNodeRuleHandle extends TrailDwsNodeRuleHandle {
	//医生与DWS表关联关系
	private final static Map<String, EngineMapping> DOCTOR_DWS_MAPPING = new HashMap<String, EngineMapping>();
	static {
		//DWS层-关联dwb_doctor
		...
		for(String key : dwsMappingSet) {
			mapping = new EngineMapping(key, "DOCTORID", "DOCTORID");
			DOCTOR_DWS_MAPPING.put(mapping.getFromIndex(), mapping);
		}
	}
	
	public DwsDoctorNodeRuleHandle(List<EngineNodeRuleGrp> grpWheres) {
		super(grpWheres);
	}
	
	public DwsDoctorNodeRuleHandle withMaster(String master) {
		this.master = master;
		return this;
	}
	...
	public String where() {
		...
		if(!master.equalsIgnoreCase(tableName)) {
			if((EngineUtil.DWB_DOCTOR.equalsIgnoreCase(master) || EngineUtil.DWB_MASTER_INFO.equalsIgnoreCase(master))
					&& DOCTOR_DWS_MAPPING.containsKey(tableName.toUpperCase())) {
				//关联dwb_doctor或dwb_master_info
				EngineMapping mapping = DOCTOR_DWS_MAPPING.get(tableName.toUpperCase());
