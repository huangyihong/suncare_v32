...
/**
 * 一次就诊互斥规则（互斥项目组中的项目作为违规项目，规则主体项目作为冲突项目）
 * @author  zhangly
 * Date: 2022年11月22日
 */
public class HiveMutexRuleHandle extends HiveRuleHandle {
	
	public HiveMutexRuleHandle(TaskProject task, TaskProjectBatch batch, Boolean trail, String datasource,
	...
	protected WithTableModel parseJudgeCondition(String fromTable) throws Exception {
		...
		sb.append("select dws_patient_1visit_itemsum.* from dws_patient_1visit_itemsum");
		...
		sb.append(" where");
		sb.append(" exists(select 1 from table_mutex x2 where x1.visitid=x2.visitid and x1.itemcode<>x2.itemcode)");
