 ...
 */
public class EngineParamFrequencyRule extends AbsEngineParamRule {	
	private MedicalDrugRule rule;
	public EngineParamFrequencyRule(MedicalDrugRule rule) {
		this.rule = rule;
	}
	...
	public String where() {
		...
		compare = rule.getTwoCompare();
		frequency = rule.getTwoFrequency();
		if(StringUtils.isNotBlank(period) 
				&& StringUtils.isNotBlank(compare) 
				&& StringUtils.isNotBlank(frequency)) {
			//频次2			
