...
@Service
@Transactional
public class ClinicalOperatioinDictMergeHandle extends AbsDictMergeHandle {
	@Autowired
...
//		wrapper.inSql("group_id", "select group_id from MEDICAL_CLINICAL_ACCESS_GROUP where instr(concat(concat(',', operations), ','), ',"+repeat+",')>0");
		wrapper.inSql("GROUP_ID", "select GROUP_ID from MEDICAL_CLINICAL_ACCESS_GROUP where locate(',"+repeat+",',concat(concat(',', operations), ','))>0");
		List<MedicalClinicalAccessGroup> dataList = service.list(wrapper);
		if(dataList.size()>0) {
			for(MedicalClinicalAccessGroup bean : dataList) {
				String value = ","+bean.getOperations()+",";
