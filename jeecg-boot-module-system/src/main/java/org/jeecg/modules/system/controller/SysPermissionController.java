...
//			String username = JwtUtil.getUsername(token);
			log.info(" ------ 通过令牌获取" + username +"拥有的访问菜单 ---- TOKEN ------ " + token);
			List<SysPermission> metaList = sysPermissionService.queryByUserSystemCode(username,sysUser.getSystemCode());
			...
			json.put("systemCode", sysUser.getSystemCode());
	...
	//@RequiresRoles({ "admin" })
	@RequiresPermissions("permission:role2menu")
	...
	private JSONObject getPermissionJsonObject(SysPermission permission) {
		...
		if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)) {
			...
			meta.put("systemCodes", permission.getSystemCodes());
			json.put("meta", meta);
		}
