...
import org.jeecg.modules.sqlxssapi.ApiAuthUtil;
...
public class SysUserController {
	...
	public Result<IPage<SysUserVO>> queryPageList(SysUser user,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        queryDepartAndRole(req, queryWrapper);

        Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);

        //如果传递的查询的querySystemCode不为空
        String querySystemCode = req.getParameter("querySystemCode");
        if(StringUtils.isNotBlank(querySystemCode)){
            String queryInSql = "select USERNAME from sys_user where SYSTEM_CODE ='" +
                    querySystemCode +"' or SYSTEM_CODE='' or SYSTEM_CODE is null ";
            queryWrapper.inSql("USERNAME" , queryInSql);
        }

		IPage<SysUser> pageList = sysUserService.page(page, queryWrapper);

		//增加角色和部门显示
        IPage<SysUserVO> pageListVO = new Page<SysUserVO>(pageNo, pageSize);
        pageListVO.setTotal(pageList.getTotal());
        List<SysUserVO> listVO = getSysUserVO(pageList.getRecords(),",");
    ...
    //根据用户信息获取部门、角色显示
    private List<SysUserVO> getSysUserVO(List<SysUser> sysUserList,String separator) {
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        List<SysUserVO> listVO = new ArrayList<>();
        for(SysUser bean:sysUserList){
            SysUserVO vo = new SysUserVO();
            BeanUtil.copyProperties(bean, vo);
            List<SysRole> roleList = sysRoleService.list(new QueryWrapper<SysRole>().
                    inSql("ID","select distinct role_id from sys_user_role where user_id='"+bean.getId()+"' ")
                    .eq("SYSTEM_CODE",user.getSystemCode()) );
            List<String> roles=roleList.stream().map(SysRole::getRoleName).collect(Collectors.toList());
            vo.setRoles(StringUtil.join(roles, separator));
            List<SysDepart> departList = sysDepartService.list(new QueryWrapper<SysDepart>().inSql("ID","select distinct dep_id from sys_user_depart where user_id='"+bean.getId()+"' "));
            List<String> departs=departList.stream().map(SysDepart::getDepartName).collect(Collectors.toList());
            vo.setDeparts(StringUtil.join(departs, separator));
            listVO.add(vo);
        }
        return listVO;
    }

    private void queryDepartAndRole(HttpServletRequest req, QueryWrapper<SysUser> queryWrapper) {
        //获取所属部门查询
        String selectedDeparts = req.getParameter("selecteddeparts");
        if(StringUtils.isNotBlank(selectedDeparts)){
            /*queryWrapper.inSql("ID","select distinct t.user_id from  sys_user_depart t where t.dep_id in " +
                    "(select distinct  t.id  from sys_depart t start with id in " +
                    "('"+selectedDeparts.replace(",","','")+"') connect by prior id=parent_id)");*/
            queryWrapper.inSql("ID","select distinct t.user_id from  sys_user_depart t where t.dep_id in " +
                    "(select t.id  from sys_depart t " +
                    "WHERE t.ORG_CODE like concat((SELECT ORG_CODE from SYS_DEPART WHERE id in " +
                    "('"+selectedDeparts.replace(",","','")+"')" +
                    "),'%') " +
                    ")");
        }
        //获取所属角色查询
        String selectedRoles = req.getParameter("selectedroles");
        if(StringUtils.isNotBlank(selectedRoles)){
            queryWrapper.inSql("ID","select distinct user_id from sys_user_role where role_id  in ('"+selectedRoles.replace(",","','")+"')");
        }
    }
	...
	public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
		...
		try {
            ...
            //更新用戶禁止访问的url列表
            this.updateUserAuthUrl(user.getUsername());
			result.success("添加成功！");
		} catch (Exception e) {
		...
		try {
            ...
            for (String id : arr) {
				if(oConvertUtils.isNotEmpty(id)) {
                    ...
                    if(CommonConstant.USER_UNFREEZE.equals(status)){
                        updateWrapper.setSql("UPDATE_PWD_TIME = null");
                        redisUtil.del(LoginController.LOGIN_FAIL_COUNT_KEY + id);
                    }
    ...
    public Result<List<String>> queryUserRole(@RequestParam(name = "userid", required = true) String userid) {
        ...
        List<SysUserRole> userRole = sysUserRoleService.list(new QueryWrapper<SysUserRole>().lambda().eq(SysUserRole::getUserId, userid).inSql(SysUserRole::getRoleId,"select ID from sys_role where system_code='"+user.getSystemCode()+"' "));
     ...
     * @return
     */
    @RequiresPermissions("permission:role2user")
    ...
    public Result<String> addSysUserRole(@RequestBody SysUserRoleVO sysUserRoleVO) {
        ...
        try {
            ...
            //更新拥有角色的用戶禁止访问的url列表
            this.updateRoleAuthUrl(sysRoleId);
     ...
     * @return
     */
    @AutoLog(value = "用户信息-更新基础信息")
    @ApiOperation(value="用户信息-更新基础信息", notes="用户信息-更新基础信息")
    @PutMapping(value = "/updateBaseInfo")
    public Result<?> updateBaseInfo(@RequestBody SysUser user) {
        ...
        Result result = Result.ok(sysUserService.getById(loginUser.getId()));
     ...
     * @return
     */
    @GetMapping("/getLoginUserByToken")
    public Result<?> getLoginUserByToken(HttpServletRequest request) {
        LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
    ...
    public void exportExcel(HttpServletRequest req, HttpServletResponse response, SysUser sysUser) throws Exception {
        ...
        try {
            ...
            List<SysUserVO> listVO = getSysUserVO(list,";");
            String suffix = ExcelTool.OFFICE_EXCEL_2010_POSTFIX;//导出文件的后缀xlsx、xls
            sysUserService.exportExcel(listVO, os, suffix);
        } catch (Exception e) {
            throw e;
        }
    }
    ...
    public Result<?> importExcelNew(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            return this.sysUserService.importExcelNew(file, multipartRequest);
    ...
    //更新用戶禁止访问的url列表
    public void updateUserAuthUrl(String username){
        ApiAuthUtil apiAuthUtil = SpringContextUtils.getBean(ApiAuthUtil.class);
        apiAuthUtil.run();
        apiAuthUtil.getNoneApis(username,false);
    }

    //更新拥有角色的用戶禁止访问的url列表
    public void updateRoleAuthUrl(String roleId){
        List<SysUser> list = sysUserService.getUserByRoleId(roleId,null);
        ApiAuthUtil apiAuthUtil = SpringContextUtils.getBean(ApiAuthUtil.class);
        apiAuthUtil.run();
        for(SysUser user:list){
            apiAuthUtil.getNoneApis(user.getUsername(),false);
        }
    }
