...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * org.jeecg.modules..*.*Controller.*(..)) || execution(public * com.ai.modules..*.*Controller.*(..))")
            ...
            if (((Result) result).getResult() instanceof IPage) {
                ...
                for (Object record : ((IPage) ((Result) result).getResult()).getRecords()) {
                    ...
                    for (Field field : oConvertUtils.getAllFields(record)) {
                        ...
                        } else if (field.getAnnotation(MedicalDict.class) != null) {
                            String code = field.getAnnotation(MedicalDict.class).dicCode();
                            String key = String.valueOf(item.get(field.getName()));

                            //翻译字典值对应的txt
                            String textValue = translateMedicalDictValue(code, key);

                            log.debug(" 医疗字典Val : " + textValue);
                            log.debug(" __翻译医疗字典字段__ " + field.getName() + CommonConstant.DICT_TEXT_SUFFIX + "： " + textValue);
                            item.put(field.getName() + CommonConstant.DICT_TEXT_SUFFIX, textValue);
                        }
