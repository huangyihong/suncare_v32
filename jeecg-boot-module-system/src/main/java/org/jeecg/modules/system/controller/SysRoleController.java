...
public class SysRoleController {
	...
	public Result<List<SysRole>> queryall() {
		...
		List<SysRole> list = sysRoleService.list(new QueryWrapper<SysRole>().eq("SYSTEM_CODE",user.getSystemCode()));
	...
	public Result<Map<String,Object>> queryTreeList(HttpServletRequest request) {
		...
		try {
			...
			//根据当前系统过滤菜单
			if(StringUtils.isNotBlank(user.getSystemCode())){
				query.and(qr ->
						qr.like(SysPermission::getSystemCodes, user.getSystemCode()).
								or().
								isNull(SysPermission::getSystemCodes).
								or().
								eq(SysPermission::getSystemCodes,"")
				);
			}
