...
public class testMainBusi2 {

	public static void main(String[] args) {
		//SpringApplication.run(EngineMain.class, args);
		LocalDateTime startTime = LocalDateTime.now();
		try {
			new EngineSpringApplication(null, new Class<?>[] { testMainBusi2.class }).run(args);
			...
			for(HisMedicalFormalBusi bean: busiList){
				...
				if (caseIdList.size() > 0){
					...
					/*String expr = "stats(" + EngineUtil.MEDICAL_UNREASONABLE_ACTION + ",q=\"" + q + "\",fq=\"" + timeJoin +"\"" +
							",count(*), sum(TOTALFEE))";*/
					String expr = "facet(" + EngineUtil.MEDICAL_UNREASONABLE_ACTION + ",q=\"" + q + "\",fq=\"" + timeJoin +"\"" +
							",bucketSorts=\"ORGNAME asc\",count(*),rows=99999999,buckets=\"ORGNAME\",count(*), sum(TOTALFEE))";
					exprList.add(expr);
				}
			...
			String path = "E:/ASIAProject/suncare_v3/excel/2019业务组结果统计-业务组机构病例"+ batchId +".xls";
			File file = new File(path);
			if (!file.getParentFile().exists()) {
				file.getParentFile().mkdirs();
			}

			String[] titles = {"业务名称","医疗机构","病例数","涉及总金额"};
...
//        String[] fields = {"ORGID","ORGNAME", "count(*)", "sum(FEE)"};
			...
			for(int busiI = 0; busiI < busiList.size(); busiI++ ){
				String expr = exprList.get(busiI);
				String busiName = busiList.get(busiI).getBusiName();
				...
				SolrUtil.stream(expr, (map, index) -> {
					...
					for(int i = 0; i < len; i++){
						Object val = i ==0?busiName:map.get(fields[i]);
						if(val != null) {
